...
def attendance_in(db:Session, entry: schemas.AttendanceEntryCreate):
    db_entry = models.AttendanceEntries(rfid_key=entry.rfid_key, in_time=entry.in_time, updated_time=datetime.datetime.now())
    db_entry.user_id = 2
    db.add(db_entry)
    db.commit()
    db.refresh(db_entry)
...
def create_attendance_out_entry(db:Session, entry: schemas.AttendanceOut):
    db_entry = db.query(models.AttendanceEntries).filter(models.AttendanceEntries.id == entry.id).first()
